...
var SceneMain = React.createClass({
  ...
  getInterprebpDownload(bpLevelNum) {
    ...
    var bodyDic={};
    bodyDic.verifyToken = this.state.AccessToken;
    bodyDic.queueNum = '103';
    bodyDic.interpretationkeys = '{"MeasureFrequency":"","Solarterms":"'+ jieqi +'","Hr":"' +this.state.Hr+ '", "PhoneDataId":"' +this.state.PhoneDataId+ '", "TargetUserId":"' +this.state.TargetUserId+ '", "Who":"' + bpLevel + '"}';

    //alert('interpretationkeys:'+bodyDic.interpretationkeys);

    ihealth.log("getInterprebpDownload", bodyDic.interpretationkeys);
    ihealth.log("getInterprebpDownload", '年：'+nDate+'月：'+yDate+'日：'+rDate);
    bodyDic.AppGuid = this.state.AppGuid;
    bodyDic.PhoneID = this.state.PhoneID;
  ...
  loginVerification() {
    MHPluginSDK.loadInfoCallback(info => {
      ...
      if ((deviceInfo != null) && (deviceInfo.userLoginFlag == true) && (deviceInfo.userID === MHPluginSDK.userId))
      {
        //用户已经登录过且userID一致，直接开始下载用户数据
        ihealth.log("loginVerification", '用户已经登录过，开始下载用户数据！');
        //alert('用户已经登录过，开始下载用户数据！');

        this.loadBPDataResult();
      }
      else
      {
        //用户未在本地登录过，第一次登录
  ...
  loadBPDataResult(){
    ...
    MHPluginSDK.loadInfoCallback(info => {
      if (info != null) {
        deviceInfo = info;
        this.setState({'PhoneDataId': deviceInfo.resultDataID,
          'TargetUserId': parseInt(deviceInfo.resultUserID),
          'HighP': parseInt(deviceInfo.resultHighP.replace('"','')),
          'LowP': parseInt(deviceInfo.resultLowP.replace('"','')),
          'Mood': parseInt(deviceInfo.resultMood),
          'TakePill': parseInt(deviceInfo.resultTakePill),
          'Hr': parseInt(deviceInfo.resultHeartRate.replace('"','')),
        ...
        //根据血压计算血压等级
        var bpLevelNum = bpDataLevel(this.state.HighP, this.state.LowP);
        //根据血压等级设置血压和等级标签字体颜色
        this.setLabelColor(bpLevelNum);
        //设置血压等级
        this.setBPResultLevel(bpLevelNum);
        //根据服药和心情设置图片显示
        this.setMoodAndMedicinePicture(this.state.Mood, this.state.TakePill);
        //下载血压个性解读信息
  ...
  userLogin() {
    ...
    userThirdPartyLogin(bodyDic, (isSuccess, responseData, errorStatus)=>{
          ...
          if (isSuccess) {
            ...
            ihealth.log("userLogin", '设备开始下载数据！');
  ...
  setBPResultLevel(bpLevelNum) {
    ...
    ihealth.log("setBPResultLevel", '血压等级是： '+'5');
  },
  showView() {
    var deviceWidth = Dimensions.get('window').width;
  ...
  bpLevelViewItch() {
    ...
    if (deviceWidth == 320) {
      return(
          <View style={{flex: 6, backgroundColor: '#ffffff', flexDirection: 'column'}}>
      ...
      return(
          <View style={{flex: (deviceWidth > 414)?8:6, backgroundColor: '#ffffff', flexDirection: 'column'}}>
...
var route = {
  ...
  renderNavRightComponent: function(route, navigator, index, navState) {
    return (
        <View style={{left:0, width:29+15*2, height:55, justifyContent:'center', alignItems:'center'}}>
        </View>
    );
  }
}
