...
function setDeviceConfig(bodyDic, callBack)
{
    var timer;
    ...
    timer = this.setTimeout(()=>{
       ...
       ihealth.log("setDeviceConfig", '上传配置-----网络访问超时！！！');
     ...
     ihealth.log("setDeviceConfig--configureInfo", JSON.stringify(configureInfo));
     MHPluginSDK.callThirdPartyAPI("10021",[],configureInfo, (code, response) => {

        ihealth.log("setDeviceConfig--code:" + code,'response:' + JSON.stringify(response));

        if (!timeoutFlag && code === 0 && response) {
          {
              ihealth.log('setDeviceConfig', '无超时');
              //取消登录超时
              this.clearTimeout(timer);

              var responseData = JSON.parse(JSON.parse(response).result);
              ihealth.log("setDeviceConfig--:", JSON.stringify(responseData));
...
/*
     ...
     //获取请求配置信息
     webSiteStr = 'https://api.ihealthlabs.com.cn:8443/Bpm1Api/setDeviceConfig.htm';
     bodyStr = toQueryString({
        'sc':'7c789858c0ec4ebf8189ebb14b6730a5',
        'sv':'399027b443041d4b93b6570576318a8e'
     });
