...
function userBpmBind(bodyDic, callBack)
{
    var timer;
    ...
    timer = this.setTimeout(()=>{
       ...
       ihealth.log("userBpmBind", '设备绑定-----网络访问超时！！！');
     ...
     ihealth.log("userBpmBind configureInfo:", JSON.stringify(configureInfo));
     MHPluginSDK.callThirdPartyAPI("10021",[],configureInfo, (code, response) => {

       ihealth.log("userBpmBind code",code);
       ihealth.log("userBpmBind", JSON.stringify(response));

         if (!timeoutFlag)
         {
             ihealth.log('userBpmBind', '无超时');
...
/*
     //获取请求配置信息
     webSiteStr = 'https://api.ihealthlabs.com.cn:8443/Bpm1Api/BPM1_Bind.htm';
     ...
     ihealth.log('userBpmBind--bodyStr', bodyStr)
    ...
    fetch(webSiteStr, {
      ...
      .then((responseData) => {
          ...
          if (!timeoutFlag)
          {
              //取消登录超时
              this.clearTimeout(timer);
              ihealth.log("userBpmBind", JSON.stringify(responseData));
